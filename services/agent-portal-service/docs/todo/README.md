# Agent Portal Service — Gap Analysis & MVP Task List

> Generated by comparing [portal-backend-FRD.md](../portal-backend-FRD.md) against the current codebase.

---

## Executive Summary

| Area | FRD Status | Code Status | Gap Severity |
|------|-----------|-------------|-------------|
| 🔐 **Authentication & Sessions** | Fully spec'd | ✅ Implemented | 🟢 Low |
| 📋 **Onboarding Workflow** | 5-step wizard | ✅ Implemented | 🟢 Low |
| 📊 **Dashboard & Metrics** | Full spec | ✅ Implemented (BFF) | 🟢 Low |
| 💬 **Conversation Management** | Full spec | ✅ Implemented (BFF) | 🟢 Low |
| 💬 **Agent Widget / Messaging** | Full spec | ✅ Implemented (RabbitMQ) | 🟢 Low |
| 🔌 **Real-time (WebSocket)** | Full spec | ✅ Implemented | 🟢 Low |
| ✅ **Input Validation (Joi)** | Full spec | ✅ Implemented | 🟢 Low |
| 🔄 **Error Handling** | Full spec | ✅ Standardized | 🟢 Low |
| ~~🛡️ Security Hardening~~ | Full spec | 🔴 Largely Missing | ⏭️ Deferred |
| ~~⚡ Performance & Caching~~ | Full spec | 🔴 Not Implemented | ⏭️ Deferred |
| ~~📈 Observability (Metrics)~~ | Full spec | 🟡 Logging Only | ⏭️ Deferred |

---

## 🏁 MVP Scope (2 Phases, 28 Tasks)

### Phase 1 — Core (must-have)
| # | File | Area | Status |
|---|------|------|--------|
| 08 | [08-input-validation.md](./08-input-validation.md) | Validation | ✅ Done |
| 01 | [01-onboarding-workflow.md](./01-onboarding-workflow.md) | Onboarding | ✅ Done |
| 02 | [02-dashboard-metrics.md](./02-dashboard-metrics.md) | Dashboard | ✅ Done |

### Phase 2 — Essential Features
| # | File | Area | Status |
|---|------|------|--------|
| 04 | [04-realtime-websocket.md](./04-realtime-websocket.md) | WebSocket | ✅ Done |
| 07 | [07-error-handling.md](./07-error-handling.md) | Error Handling | ✅ Done |

### ⏭️ Deferred (Post-MVP)
| # | File | Area | Reason |
|---|------|------|--------|
| 03 | [03-security-hardening.md](./03-security-hardening.md) | Security | Add after core features stable |
| 05 | [05-observability.md](./05-observability.md) | Observability | Logging already works |
| 06 | [06-performance.md](./06-performance.md) | Performance | Optimize after MVP validated |

---

## Dependency Graph

```
08-input-validation ──┐
                      ├──> 01-onboarding ──> 02-dashboard
07-error-handling ────┘           │
                                  └──> 04-realtime-websocket
```

> **Deferred items** (03, 05, 06) have no MVP dependencies and can be added independently post-launch.
